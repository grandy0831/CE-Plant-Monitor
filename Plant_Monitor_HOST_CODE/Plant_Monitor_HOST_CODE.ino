...
#include <Wire.h>                  // Library for Inter-Integrated Circuit
...
void loop() {
  ...
  //Save temperature, humidity and moisture values into variables
  int dataT = Temperature;
  int dataH = Humidity;
  int dataM = Moisture;

  // Start I2C communication and send data to the device with I2C address 13
  Wire.beginTransmission(13);

  //Write temperature, humidity and moisture values to the I2C bus
  Wire.write(dataT);  
  Wire.write(dataH);  
  Wire.write(dataM);      
  ...
  //Print "plant send" on the serial monitor for debugging or confirming whether the data is sent successfully.
...
void sendMQTT() {
  ...
  snprintf (msg, 50, "%.1i", Moisture);  // Format the message
